...
export const openMeteoWeatherPlugin: WeatherPlugin = {
  ...
  async fetchForecast(input: WeatherForecastRequest) {
    ...
    if (!response.ok) {
      throw new Error(
        `Open-Meteo solar forecast request failed with HTTP ${response.status}: ${await response.text()}`,
      );
    }
...
function buildOpenMeteoUrl(input: {
  ...
  if (input.use15Minute) {
    ...
    params.set("hourly", "shortwave_radiation,temperature_2m,cloud_cover");
    params.set(
      "forecast_hours",
